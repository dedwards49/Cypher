...
#pragma rtGlobals=3		// Use modern global access method and strict wave access.
#pragma ModuleName=DE_ForceClamp
...
Static function Start()
	...
	variable MultiCTFCEngage=DE_CheckRamp("Do You Want to Sweep About?","Sweep",PopupName="popup7") //Check if you wanna sweep
...
Static Function InitandRamp()
	...
	if(StringMatch(RefoldSettings[%RefineHoldoff][0],"Yes")==1)	//If you want to refine the holdoff position visually on the graph that happens here
		...
		Results=DE_ForceClamp#Markers(HoldoffDistance)
	...
	IR_XSetInWavePair(1,"8","Cypher.Input.FastA",DefV_Slow,"Cypher.LVDT.Z",ZSensor_Slow,"DE_ForceClamp#RampDown()", -decirate)
	...
	DE_ForceClamp#GrabCTFC()			//Initialize the CTFC wave
	DE_ForceClamp#SetCTFCParms(RampInfo,RefoldSettings)//This sets up everything we need for the CTFC, including both the dwell FB loop
...
Static Function GrabCTFC()
	...
	Make/O/T root:DE_CTFC:ForceClamp_TriggerSettings
	Variable error=0
	error+=td_ReadGroup("ARC.CTFC",root:DE_CTFC:ForceClamp_TriggerSettings)
	return error
...
Static Function SetCTFCParms(RampSettings,RefoldSettings)
	...
	wave ZsensorVolts_Fast=Zsensor_fast
	Wave/T TriggerInfo=root:DE_CTFC:ForceClamp_TriggerSettings
	...
	TriggerInfo[%CallBack] = "DE_ForceClamp#CallBack()"
	...
	//This sets up an Force feedback servo to engage during the final pause, it is on FB.Bank 3 because I got that to work.
	...
	PIDSLoopGroup[%StartEvent]=num2str(12)
	PIDSLoopGroup[%StopEvent]="Never"
...
Static Function RampDown()
	...
	ir_StopPISloop(3)
	td_ws("Event.6","once")//Fires the ramp away from the surface.

	td_SetRamp(str2num(RefoldInfo[%ApproachTime][0]), "PIDSLOOP.4.setpoint", 0, newpoint, "", 0, 0, "", 0, 0, "DE_ForceClamp#UnfoldingDone()")  //This should ramp is back to the surface on PIDSLoop.4 thanks to having recorded the surface before
//	td_SetRamp(str2num(RefoldInfo[%ApproachTime][0]), "PIDSLoop.4.Setpoint", 0, newpoint, "", 0, 0, "", 0, 0, "DE_ForceClamp#UnfoldingDone()")  
end//RampDown()
...
Static Function FinalPull()
	...
	variable outdecirate=DE_Outdecirate("ForceClampFinal")
	IR_xSetOutWave(2,"15","PIDSLoop.4.SetpointOffset",CustomWave2,"DE_ForceClamp#Done()",outdecirate)
...
Static function MakeWaves(a0,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,a13,Wpar,c0,c1,c2,c3,c4,c5,c6,c7)
	...
	RefoldSettings[0][1]= {"Experiment Name","Approach Distance","Approach Time","Approach Delay","Holdoff Distance","Refine Holdoff","Force Trigger","Retract Speed","Extension Pause","FinalDistance","FinalVelocity","Surface Pos","Data Rate","UltraFast Too?"}
